...
本模板用于生成"销售订单"（Sales Order），适用于销售业务场景的打印格式。

## 模板特点

### 1. 文档结构

- **文档类型**：销售订单（Sales Order）
- **基本信息**：客户名称、订单编号、客户订单、订单日期、关联项目
- **物料明细**：序号、物料信息、交货日期、数量、单价、金额
- **金额汇总**：小计、税费、总计
- **备注区域**：大写金额、备注/条款（包含付款条款）
- **签名区域**：销售员、销售主管、财务审核、客户确认

### 2. 主要功能

#### 基本信息显示
- **客户名称**：显示客户名称（`doc.customer_name`）
- **订单编号**：显示文档编号（`doc.name`）
- **客户订单**：显示客户订单号和日期（`doc.po_no` 和 `doc.po_date`）
  - 如果存在订单日期，显示格式：`客户订单号 (订单日期)`
  - 如果不存在订单日期，仅显示客户订单号
- **订单日期**：显示交易日期（`doc.transaction_date`），使用 `frappe.utils.format_date` 格式化
- **关联项目**：自动查找并显示关联项目
...
#### 物料明细表格
- **序号**：自动编号（`loop.index`）
- **物料信息**：显示物料代码、物料名称、描述（如果存在）、客户物料编号（如果存在）
- **交货日期**：显示交货日期（`item.delivery_date`），使用 `frappe.utils.format_date` 格式化
- **数量**：显示销售数量（`item.qty`）和单位（`item.uom`）
...
#### 备注区域
- **大写金额**：显示金额大写（`doc.in_words`），不换行显示
- **备注/条款**：单独一行显示
  - 如果存在付款条款模板（`doc.payment_terms_template`），优先显示付款条款
  - 优先显示条款（`doc.terms`），如果不存在则显示备注（`doc.remarks`）

#### 签名区域
- **销售员**：显示文档创建者（`doc.owner`）
- **销售主管**：空白签名行
- **财务审核**：空白签名行
- **客户确认 (盖章)**：空白签名行
...
- `doc.remarks`：备注信息
- `doc.terms`：条款信息（富文本）
- `doc.payment_terms_template`：付款条款模板
- `doc.in_words`：金额大写
- `doc.currency`：币种
...
- `doc.items`：物料明细列表
  ...
  - `item.item_name`：物料名称
  - `item.description`：物料描述
  - `item.customer_item_code`：客户物料编号
...
- `po_display`：客户订单显示文本（格式：`客户订单号 (订单日期)`）
...
#### 物料明细表格列
- **序号**：自动编号（1, 2, 3, ...）
- **物料信息**：物料代码（粗体）、物料名称、描述（如果存在且不同于名称）、客户物料编号（如果存在，格式：`[客]: 客户物料编号`）
- **交货日期**：格式化后的日期
- **数量**：销售数量（粗体）+ 单位
- **单价**：格式化后的单价
- **金额**：格式化后的金额（粗体）
...
#### 备注区域布局
- **大写金额**：与金额汇总表格并排显示（左侧60%，右侧40%），使用 `white-space: nowrap` 确保不换行
- **备注/条款**：单独一行显示，支持内容换行和自适应高度
...
1. **模板位置**：`print_format/sale-order-standard/template.html`
2. **样式文件**：`print_format/sale-order-standard/styles.css`
3. **在Frappe中配置**：
   - 创建或编辑打印格式
   - 选择"HTML"类型
...
- **大写金额**：使用 `white-space: nowrap` 确保不换行
- **备注/条款**：单独一行显示，支持内容换行
