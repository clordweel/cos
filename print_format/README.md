...
- **收货信息**：与上方基本信息**同一张** `info-table`，排版与「关联项目」等行一致：`info-label` + `info-value`（`收货地址:` 标签不加粗）。次行两列：「收货方:」「电话:」。供应商侧联系人为「发货方:」。字段优先取 `shipping_address` 对应 `Address.custom_address_display`，否则用 `shipping_address_display`；联系人与电话取自 `custom_shipping_contact_person`、`custom_shipping_contact_phone`（无则整块「收货信息」不显示）。
