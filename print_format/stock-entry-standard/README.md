...
本模板用于生成"移库单"（Stock Entry），适用于库存转移、物料调拨等场景的打印格式。

## 模板特点

### 1. 文档结构

- **文档类型**：移库单（Stock Entry）
- **基本信息**：移库单号、过账日期、移库类型、项目、关联来源
- **物料明细**：序号、物料信息、源仓库、目标仓库、批次/序列号、数量、确认框
- **备注区域**：备注信息
- **签名区域**：制单人、发货人(源)、收货人(目标)、审核/监管

### 2. 主要功能

#### 基本信息显示
- **移库单号**：显示文档编号（`doc.name`）
- **过账日期**：显示过账日期（`doc.posting_date`），使用 `frappe.utils.format_date` 格式化
- **移库类型**：显示移库类型（`doc.stock_entry_type`），使用 `_()` 函数进行翻译
- **项目**：自动查找并显示关联项目
  - 优先级1：从主表获取项目ID（`doc.project`）
  - 优先级2：从子表第一行获取项目ID（`doc.items[0].project`）
  - 优先级3：从关联的工单获取项目ID（如果存在 `doc.work_order`）
  - 显示格式：`项目ID (项目名称)` 或 `项目ID`
- **关联来源**：自动识别并显示关联文档
  - 优先显示生产工单（`doc.work_order`）
  - 其次显示BOM（`doc.bom_no`）
  - 最后显示采购订单（`doc.purchase_order`）

#### 物料明细表格
- **序号**：自动编号（`loop.index`）
- **物料信息**：显示物料代码、物料名称、描述（如果存在）
- **源仓库**：显示源仓库名称（`item.s_warehouse`）
- **目标仓库**：显示目标仓库名称（`item.t_warehouse`）
- **批次/序列号**：显示批次号（`item.batch_no`）和序列号（`item.serial_no`）
- **数量**：显示移库数量（`item.qty`）和单位（`item.uom`）
- **确认框**：用于确认的复选框
...
##### 基础字段
- `doc.name`：文档编号（移库单号）
- `doc.posting_date`：过账日期
- `doc.stock_entry_type`：移库类型（需要翻译）
...
- `doc.bom_no`：BOM编号（如果存在）
- `doc.purchase_order`：采购订单（如果存在）
- `doc.items`：物料明细列表
  - `item.item_code`：物料代码
  - `item.item_name`：物料名称
  - `item.description`：物料描述
  - `item.s_warehouse`：源仓库
  - `item.t_warehouse`：目标仓库
  - `item.batch_no`：批次号
  - `item.serial_no`：序列号
  - `item.qty`：移库数量
  - `item.uom`：单位
...
##### 计算字段
- `source_label`：关联来源标签（"生产工单"、"BOM"、"采购订单"或"无"）
- `source_name`：关联来源名称
...
2. **优先级2**：BOM（`doc.bom_no`）
   - 标签：`"BOM"`
   - 名称：`doc.bom_no`

3. **优先级3**：采购订单（`doc.purchase_order`）
   - 标签：`"采购订单"`
   - 名称：`doc.purchase_order`
...
1. **模板位置**：`print_format/stock-entry-standard/template.html`
2. **样式文件**：`print_format/stock-entry-standard/styles.css`
3. **在Frappe中配置**：
   - 创建或编辑打印格式
   - 选择"HTML"类型
