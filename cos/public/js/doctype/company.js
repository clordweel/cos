...
function add_initialize_defaults_button(frm) {
	frm.add_custom_button(__('初始化默认值'), function () {
		// 先获取可用的配置文件列表
		frappe.call({
			method: 'cos.cos_accounts.controllers.company.get_available_config_files',
			freeze: true,
			callback: function(r) {
				if (r.message && r.message.length > 0) {
					// 构建配置文件选项（字符串数组格式，Frappe Select 字段要求）
					const config_options = r.message.map(function(config) {
						return config.display_name;
					});
					
					// 创建显示名称到文件名的映射
					const display_name_to_filename = {};
					r.message.forEach(function(config) {
						display_name_to_filename[config.display_name] = config.filename;
					});
					
					// 显示对话框，让用户选择配置文件和是否覆盖已有值
					const dialog = new frappe.ui.Dialog({
						title: __('初始化默认值'),
						fields: [
							{
							...
							},
							{
								fieldtype: 'Select',
								label: __('配置文件'),
								fieldname: 'config_file_display',
								options: config_options,
								default: r.message[0].display_name, // 默认选择第一个（通常是默认文件）
								description: __('选择要使用的默认值配置文件'),
								reqd: 1
							},
							{
								fieldtype: 'Check',
						...
						primary_action: function () {
							...
							const config_file_display = dialog.get_value('config_file_display');
							// 通过映射获取实际的文件名
							const config_file = display_name_to_filename[config_file_display];
							dialog.hide();
