...
- **收货信息**：`info-table` 两列宽表线样式。首行整宽：第一行「收货地址:」、第二行地址正文；次行两列：「联系人:」「电话:」。字段优先取 `shipping_address` 对应 `Address.custom_address_display`，否则用 `shipping_address_display`；联系人与电话取自 `custom_shipping_contact_person`、`custom_shipping_contact_phone`（无则整块「收货信息」不显示）。
